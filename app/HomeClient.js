        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {loading ? (
            ...
            <div className="col-span-full py-48 text-center bg-[#0A0A0A]/40 backdrop-blur-3xl rounded-[4rem] border border-white/10 shadow-2xl relative overflow-hidden">
              ...
              <button 
                onClick={clearFilters}
                className="px-12 py-5 bg-white text-black hover:bg-[#185FA5] hover:text-white rounded-full text-[11px] font-black uppercase tracking-[0.3em] transition-all"
              >
                {t.showAll}
              </button>
            </div>
            ...
            listings.map((listing, index) => (
              <Link key={listing.id} href={`/oglas/${listing.id}`} className="block group listing-card">
